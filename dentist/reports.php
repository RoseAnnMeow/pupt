          ...
          ?>
            ...
            <div class="card card-primary card-outline">
                ...
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-2">
                        <button type="button" id="print_medicines" class="btn btn-success btn-sm btn-flat btn-block">Generate  PDF</button>
                    </div>
                  </div>
              </div>
            </div>
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title">Supply Inventory</h3>
                </div>
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-2">
                        <button type="button" id="print_supplies" class="btn btn-success btn-sm btn-flat btn-block">Generate  PDF</button>
                    </div>
                  </div>
              </div>
            </div>
...
<script>
    $(document).ready(function () {
      var table = $('#transact-table').DataTable({
        ...
        $("#print_medicines").click(function() {
    
            var win = window.open("print-medicine-inventory.php?_blank");
            if(win) {
              win.focus();
            } else {
              showCustomMessage('Please allow popups.');
        ...
        $("#print_supplies").click(function() {
            var win = window.open("print-supply-inventory.php?_blank");
            if(win) {
              win.focus();
            } else {
              showCustomMessage('Please allow popups.');
