                ...
                <div class="card-body">
                  <table id="pres" class="table table-borderless table-hover">
                    ...
                    <tbody>
                      <?php
                        $i = 1;
                        $sql = "SELECT CONCAT(pat.lname,', ',pat.fname) as pname,doc.name,pres.medicine,pres.advice,pres.date,pres.id,m.med_name ,pres.dose,pres.duration
                        FROM prescription pres 
                        JOIN tblpatient pat ON pat.id = pres.patient_id 
                        JOIN tbldoctor doc ON doc.id = pres.doc_id 
                        JOIN medicines m  ON m.id = pres.medicine 
