                           ...
                           <div class="tab-pane fade" id="treatment" role="tabpanel" aria-labelledby="treatment-tab">
                                 <table id="treatment-table" class="table table-hover" style="width:100%;">
                                       ...
                                       <tbody>
                                          <?php
                                             $id = $_SESSION['auth_user']['user_id']; 
                                             $sql = "SELECT CONCAT(p.fname,'',p.lname) as pname,t.id,t.teeth,t.complaint,t.treatment,t.fees,t.remarks,s.day FROM treatment t INNER JOIN tblpatient p ON t.patient_id = p.id INNER JOIN schedule s ON s.id = t.visit WHERE patient_id ='$id' ORDER BY id DESC";
                                             $query_run = mysqli_query($conn,$sql);
                                             while($row = mysqli_fetch_array($query_run)){
                                          ?>
                                          <tr>                                   
